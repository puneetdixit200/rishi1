...
def _user_read(user: User, scope: ScopeContext) -> UserRead:
    return UserRead(
        id=user.id,
...
def _account_scope_is_active(db: Session, user: User) -> bool:
    ...
    return bool(
        ...
        and company.business_group_id == user.business_group_id
    )
...
def login(
    ...
    if not user.is_active or not _account_scope_is_active(db, user):
        ...
        write_audit_log(
            ...
            new_value_json={"email": credentials.email, "success": False},
            request=request,
            notes="Inactive or invalid account scope login attempt.",
            commit=True,
        )
...
def step_up_authentication(
    ...
    if not verify_password(payload.password, current_user.password_hash):
        write_audit_log(
            db,
            action="auth.step_up_failure",
            entity_type="user",
            entity_id=current_user.id,
            user=current_user,
            request=request,
            notes="Step-up password verification failed.",
            commit=True,
        )
    ...
    write_audit_log(
        db,
        action="auth.step_up_success",
        entity_type="user",
        entity_id=current_user.id,
        user=current_user,
        new_value_json={"last_step_up_at": current_user.last_step_up_at.isoformat()},
        request=request,
        commit=True,
    )
...
def read_current_user(
    ...
    return _user_read(current_user, scope)
