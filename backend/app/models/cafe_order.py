...
class CafeOrder(CompanyScopeMixin, TimestampMixin, Base):
    ...
    guest_access_id: Mapped[int | None] = mapped_column(
        ForeignKey("cafe_guest_access.id"), nullable=True
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "company_id", "idempotency_key_hash", name="uq_cafe_orders_company_idempotency"
        ),
...
class CafeOrderItem(CompanyScopeMixin, TimestampMixin, Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    branch_id: Mapped[int] = mapped_column(ForeignKey("branches.id"), nullable=False)
    cafe_order_id: Mapped[int] = mapped_column(
        ForeignKey("cafe_orders.id", ondelete="CASCADE"), nullable=False
    )
    ...
    created_by: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
    billed_invoice_item_id: Mapped[int | None] = mapped_column(
