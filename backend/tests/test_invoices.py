...
def test_draft_invoice_does_not_reduce_stock_and_issue_reduces_stock(
    client,
    ...
    with db_session_factory() as db:
        inventory = db.scalar(
            select(Inventory).where(Inventory.product_id == ids["product_id"], Inventory.branch_id == ids["branch_id"])
        )
    ...
    issue_response = client.post(
        ...
        json={"payments": [{"payment_mode_id": ids["cash_mode_id"], "amount": "236.00"}]},
    ...
    assert issued["paid_amount"] == "236.00"
    ...
    assert sale.total_amount == Decimal("236.00")


def test_credit_invoice_creates_customer_receivable_ledger(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
    ...
    assert invoice["balance_due"] == "118.00"
    with db_session_factory() as db:
    ...
    assert ledger_entry.debit == Decimal("118.00")
    assert ledger_entry.credit == Decimal("0.00")


def test_invoice_payment_collection_updates_balance_and_customer_ledger(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
...
def test_invoice_number_is_unique_across_checkouts(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
    ...
    assert first.json()["invoice_number"] == "GST-2026-0001"
    assert second.json()["invoice_number"] == "GST-2026-0002"
    with db_session_factory() as db:
        assert db.scalar(select(func.count()).select_from(Invoice)) == 2


def test_analyst_cannot_create_invoice(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
