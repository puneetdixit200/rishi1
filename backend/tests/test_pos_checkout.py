...
def test_pos_product_search_finds_barcode_and_returns_branch_stock(
    client,
...
def test_pos_quote_returns_backend_calculated_totals_without_creating_invoice(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
    ...
    assert quote["cgst_total"] == "17.10"
    assert quote["sgst_total"] == "17.10"
    assert quote["grand_total"] == "224.20"
    assert quote["items"][0]["quantity_on_hand"] == "20.00"
    with db_session_factory() as db:
        assert db.scalar(select(func.count()).select_from(Invoice)) == 0


def test_pos_checkout_creates_invoice_payment_stock_movement_and_sales_compatibility(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
    ...
    assert summary_response.json()["tax_total"] == "18.00"
    ...
    assert payment.amount == Decimal("118.00")
    assert sale is not None


def test_pos_checkout_rejects_insufficient_stock_atomically(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory, quantity="1.00")
    payload = checkout_payload(ids, quantity="2.00")
    payload["payments"][0]["amount"] = "236.00"
...
def test_pos_credit_checkout_creates_customer_ledger_without_payment_row(
    client,
    db_session_factory: sessionmaker[Session],
) -> None:
    ids = setup_invoice_data(client, db_session_factory)
    payload = {
        **checkout_payload(ids),
